...
const Dashboard = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      // Cargamos los enlaces del array links
      setLinks(res.data.links || []);
      
      // Cargamos la configuración (profile, socials y theme)
      setSettings({
        profile: { 
  ...
  const handleDeleteLink = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({ icon: "success", title: "¡Borrado!", showConfirmButton: false, timer: 1000 });
      } catch (err) {
  ...
  // --- Gestión de Perfil, Redes y Colores ---
  const handleSaveSettings = async () => {
    try {
      // Mandamos el objeto settings completo (incluye profile, theme y socials)
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      setSettings({ ...settings, profile: { ...settings.profile, avatarUrl: res.data.url } });
  ...
  const copyToClipboard = () => {
    ...
    Swal.fire({ icon: "info", title: "URL Copiada", text: "Pégala en tu bio de Instagram", timer: 2000 });
  };
          ...
          <Row className="justify-content-center">
            <Col md={6}>
              <Card className="shadow-sm border-0 p-4">
                ...
                <div className="mb-4">
                  <Form.Label className="fw-bold">Redes Sociales (URLs)</Form.Label>
                  <Form.Group className="mb-2">
                    <div className="input-group">
                      <span className="input-group-text"><FaInstagram /></span>
                      <Form.Control placeholder="https://instagram.com/tu-usuario" value={settings.socials.instagram} onChange={(e) => setSettings({ ...settings, socials: { ...settings.socials, instagram: e.target.value } })} />
                    </div>
                  </Form.Group>
                  <Form.Group className="mb-2">
                    <div className="input-group">
                      <span className="input-group-text"><FaGithub /></span>
                      <Form.Control placeholder="https://github.com/tu-usuario" value={settings.socials.github} onChange={(e) => setSettings({ ...settings, socials: { ...settings.socials, github: e.target.value } })} />
                    </div>
                  </Form.Group>
                </div>

                <Row className="mb-3 text-center">
                  <Col><Form.Label className="fw-bold d-block">Fondo</Form.Label><Form.Control type="color" className="mx-auto" value={settings.theme.backgroundColor} onChange={(e) => setSettings({ ...settings, theme: { ...settings.theme, backgroundColor: e.target.value } })} /></Col>
                  <Col><Form.Label className="fw-bold d-block">Botones</Form.Label><Form.Control type="color" className="mx-auto" value={settings.theme.buttonColor} onChange={(e) => setSettings({ ...settings, theme: { ...settings.theme, buttonColor: e.target.value } })} /></Col>
                </Row>

                <Button variant="primary" className="w-100 fw-bold py-2" onClick={handleSaveSettings}><FaSave className="me-2" /> GUARDAR TODO</Button>
              </Card>
            </Col>
          </Row>
        </Tab>
